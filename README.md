...
```
Unfortunantly the paths for configured database are saved inside the singularity image so it is important that you keep the debug file generated by the configuration so the database paths can be used when runing EnTap.

The path the debug file will be in the Stdout(on the screen) and will look like this.
